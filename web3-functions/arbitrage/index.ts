...
const configs = [
    ...
    },
    {
        ...
        amounts: [BigNumber.from(1).mul(BigNumber.from(10).pow(18)), BigNumber.from(10).mul(BigNumber.from(10).pow(18)), BigNumber.from(100).mul(BigNumber.from(10).pow(18))], // 1 eth, 10 eth and 100 eth
...
Web3Function.onRun(async (context: Web3FunctionContext) => {
    ...
    for (const cfg of configs) {
        ...
        for (const amount of cfg.amounts) {
            console.log(`Trying amount ${amount.toString()} on ${cfg.token} arb contract`);
            try {
                ...
                console.log(`Swapping amount ${amount.toString()} on ${cfg.token} arb contract failed:`, {error: e.error.body});
